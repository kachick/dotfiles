...
#   passthru = {
#     updateScript = gnome.updateScript { packageName = "gnome-session"; };
#     providedSessions = [
#       "gnome"
#       "gnome-xorg"
#     ];
...
{ edge-pkgs, substituteAll }:
edge-pkgs.gnome-session.overrideAttrs {
  patches = [
    (substituteAll {
      src = ./fix-paths.patch;
      gsettings = "${edge-pkgs.glib.bin}/bin/gsettings";
      dbusLaunch = "${edge-pkgs.dbus.lib}/bin/dbus-launch";
      bash = "${edge-pkgs.bash}/bin/bash";
    })
  ];
}
