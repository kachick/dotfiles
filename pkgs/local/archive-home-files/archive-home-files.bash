...
# Use the 'home-manager' command provided via runtimeInputs in Nix.
# This ensures we use the same version as the one configured in the flake.
hm_generation_path=$(home-manager generations | rg 'id \d+ -> /nix/store' | head -n1 | cut -d ' ' -f 7)

if [ -z "$hm_generation_path" ]; then
	echo "Error: Could not find any home-manager generations."
	exit 1
fi
